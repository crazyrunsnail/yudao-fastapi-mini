...
def init_db():
    ...
    # We call the python script directly using the current python interpreter
    # assuming the user has activated the venv OR we explicitly use the venv python
    ...
    if venv_python.exists():
        ...
        # Fallback to system python if venv not found (might fail if deps missing)
...
def dev_backend():
    ...
    if IS_WINDOWS:
        uvicorn_cmd = str(BACKEND_DIR / ".venv" / "Scripts" / "uvicorn")
    else:
        uvicorn_cmd = str(BACKEND_DIR / ".venv" / "bin" / "uvicorn")
    
    # Check if venv exists, otherwise try global uvicorn or fail gracefully
    cmd = f"{uvicorn_cmd} main:app --reload"
    if not Path(uvicorn_cmd).exists():
         print("Virtual environment not found. Using system 'uvicorn'...")
